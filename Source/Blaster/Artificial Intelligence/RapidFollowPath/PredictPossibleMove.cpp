...
UPredictPossibleMove::UPredictPossibleMove()
{
	SetupDefaultObject();
}

void UPredictPossibleMove::SetupDefaultObject()
{
...
*/
void UPredictPossibleMove::AutoPredictPossibleMoveFromLatestData()
{
...
void UPredictPossibleMove::PreloadObject(AActor* Object, FVector CharacterLocation, FVector ClosestObjectLocation, FRotator CharacterRotation, FRotator ObjectRotation)
{
	...
	if (RestrictScan.IsEmpty()) SetupDefaultObject();
...
void UPredictPossibleMove::PreloadObjectFaceSide(AActor* Object, FVector CharacterLocation, FVector ClosestObjectLocation)
{
	FVector Origin;
	FVector ObjectExtent;

	FLineStruct SideLeft = FLineStruct();
	FLineStruct SideRight = FLineStruct();
...
void UPredictPossibleMove::PreloadObjectItem(AActor* Object)
{
	FVector Origin;
...
void UPredictPossibleMove::GenerateLocationRouteByObjects()
{
	/*for (int i = 0; i < LeftSideData.Num() && i < RightSideData.Num(); i++)
	{
		OptimizeFaceSideResult(LeftSideData[i], RightSideData[i]);
	}*/
}

void UPredictPossibleMove::OptimizeFaceSideResult(FLineStruct GivenLeftSide, FLineStruct GivenRightSide)
{
	FLineStruct LatestObjectFaceSide;

	FHitResult CurrentLineCollision;
	FHitResult CurrentLineCollision2;
	...
	else LatestObjectFaceSide.LocationA = GivenLeftSide.LocationB;

	if (GetWorld() && GetWorld()->LineTraceSingleByChannel(
		CurrentLineCollision2,
		GivenRightSide.LocationB,
		GivenRightSide.LocationA,
		ECollisionChannel::ECC_Visibility
	))
		LatestObjectFaceSide.LocationB = CurrentLineCollision2.ImpactPoint == FVector(0) ?
		GivenRightSide.LocationB : CurrentLineCollision2.ImpactPoint;
	else LatestObjectFaceSide.LocationB = GivenRightSide.LocationB;

	ResultObjectFaceSide.Add(LatestObjectFaceSide);
}

void UPredictPossibleMove::ShrinkLocation(FLineStruct& GivenLeftSide, FLineStruct& GivenRightSide)
{
	switch (BestObjectFaceSide)
	{
	case EObjectFaceSideType::EOFST_UP:
	case EObjectFaceSideType::EOFST_DOWN:
		GivenLeftSide.LocationA.X += SHRINK_OBJECT_NUM;
		GivenLeftSide.LocationB.X += SHRINK_OBJECT_NUM;
		GivenRightSide.LocationA.X -= SHRINK_OBJECT_NUM;
		GivenRightSide.LocationB.X -= SHRINK_OBJECT_NUM;
		break;

	case EObjectFaceSideType::EOFST_LEFT:
	case EObjectFaceSideType::EOFST_RIGHT:
		GivenLeftSide.LocationA.Y -= SHRINK_OBJECT_NUM;
		GivenLeftSide.LocationB.Y -= SHRINK_OBJECT_NUM;
		GivenRightSide.LocationA.Y += SHRINK_OBJECT_NUM;
		GivenRightSide.LocationB.Y += SHRINK_OBJECT_NUM;
		break;
...
void UPredictPossibleMove::LocateLineLocation(FVector GivenFirstLocation, FLineStruct& GivenLine)
{
...
void UPredictPossibleMove::GetClosestDistancePointFromAvailablePoint()
{
...
*/
void UPredictPossibleMove::GetPickupScore()
{
	int32 ObjMultiply;
	for (int32 i = 0; i < ResultShortestDistance.Num(); i++)
	{
		...
		{
			if ((ObjPair.Value - DistanceCloseToCharacter[i]).Size() <= DEFAULT_REQUIRE_OBJECT_DISTANCE)
			{
				++ObjMultiply;
			}
		}
		float ItemScore = (float)(DEFAULT_OBJECT_SCORE * ObjMultiply) / (ItemData.IsEmpty() ? 1 : ItemData.Num());
...
void UPredictPossibleMove::ConvertToPercent()
{
	if (!ResultShortestDistance.IsEmpty())
	{
...
void UPredictPossibleMove::PrintAllFaceSide()
{
...
void UPredictPossibleMove::PrintAllResultPoint()
{
	for (int i = 0; i < LeftSideData.Num(); i++)
	{
...
void UPredictPossibleMove::PrintAllDistancePoint()
{
...
void UPredictPossibleMove::PrintAllDistancePointPercent()
{
...
void UPredictPossibleMove::ReloadDistance2D()
{
...
* target location
*/
void UPredictPossibleMove::ApplyDistance2DToWorld()
{
...
TArray<float> UPredictPossibleMove::CheckDistanceByDijkstra()
{
...
void UPredictPossibleMove::PrintShortestValueFromDijkstra(TArray<float>PointResult)
{
...
int32 UPredictPossibleMove::MinDistanceDijkstra(TArray<float> PointResult, TArray<bool> PointCheck)
{
...
void UPredictPossibleMove::Clear()
{
	LeftSideData.Empty();
	RightSideData.Empty();
...
void UPredictPossibleMove::SetupLineObjectSideData(EObjectFaceSideType NameType, FVector ObjectLocation, FVector ObjectExtent, FVector CharacterLocation, FVector ClosestObjectLocation)
{
	//if (NumObjectFaceReceive == MAX_OBJECT_FACE_RECEIVED) return;

	FLineStruct ObjectFaceSideData;
...
* 		   d = Origin.Y + ObjectExtent.Y
*/
void UPredictPossibleMove::SetLineObjectSideData(EObjectFaceSideType NameType, FVector Location, FVector Extent, FLineStruct& ObjectSideData)
{
	switch (NameType)
	{
	case EObjectFaceSideType::EOFST_DOWN:
...
FVector UPredictPossibleMove::LineGenerator(FVector LocationA, FVector LocationB, double ZAxis)
{
...
*/
FVector UPredictPossibleMove::TwoPointFormGenerator(FVector CharacterLocation, FVector ClosestObjectLocation, FLineStruct& ObjectSideData)
{
...
FVector UPredictPossibleMove::FVectorConvertRoundDouble(FVector GivenVector)
{
	return FVector(
...
FVector UPredictPossibleMove::GetFinalMove()
{
	float MaxValue = UE_DOUBLE_SMALL_NUMBER;
	int32 MaxIndex = -1;
	for (int32 i = 0; i < ResultShortestDistance.Num(); i++)
	{
		if (MaxValue < ResultShortestDistance[i])
		{
			MaxValue = ResultShortestDistance[i];
			MaxIndex = i;
		}
	}
	return (MaxIndex == -1) ? FVector() : DistanceCloseToCharacter[MaxIndex];
}

FLineStruct UPredictPossibleMove::FLineStructConvertRoundDouble(FLineStruct GivenLine)
{
	return FLineStruct{
...
EObjectFaceSideType UPredictPossibleMove::GetCollisionObjectSide(FVector ObjectLocation, FVector ObjectExtent, FVector CharacterLocation, FVector ClosestObjectLocation)
{
...
double UPredictPossibleMove::GetDistance(FVector GivenA, FVector GivenB)
{
	return FMath::Square(
...
bool UPredictPossibleMove::IsInsideFaceSide(FVector TwoPointFormPoint, FLineStruct ObjectSideData)
{
...
bool UPredictPossibleMove::IsSingleLineXAxis(FVector A, FVector B)
{
	UE_LOG(LogTemp, Warning, TEXT("Loc: %f | %f | %f | %f"), A.X, A.Y, B.X, B.Y);
	return (A.X == B.X && A.Y != B.Y);
}

bool UPredictPossibleMove::IsObjectInRange(FRotator CharacterRotation, FRotator ObjectRotation)
{
